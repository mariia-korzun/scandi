...
const Carousel = ({
  ...
  const [sliderDotEqualCurrentSlide, setSliderDotEqualCurrentSlide] = useState(
    false
  );
  ...
  function changeOrder(direction, array) {
    ...
    return direction
      ? [array[array.length - 1], ...array.slice(0, array.length - 1)]
  ...
  useEffect(() => {
    ...
    let newCurrentSlideIndex = defineNextIndex(
      direction,
      currentSlideIndex,
      data
    );
  ...
  function handleGestureMove(e, isTouch) {
    ...
    let newShift =
      gestureStart.x - (isTouch ? e.touches[0].clientX : e.clientX);
  ...
  function handleGestureEnd() {
    ...
    if (
      Math.abs(shift) > MAX_SHIFT ||
      (swipeTime < MAX_SWIPE_TIME && Math.abs(shift) > MIN_SHIFT)
    ) {
  ...
  function initiateSlideChange({
    ...
    if (updateDots) {
      setSliderDotEqualCurrentSlide(updateDots);
    }
  ...
  for (let i = 0; i <= 2; i++) {
    array.push(
      <Slide style={classes[i].style} key={i}>
        {fetchedData ? (
          <Content data={data[indexes[i].index]} />
        ) : (
...
Carousel.propTypes = {
  fetchedData: PropTypes.oneOfType([PropTypes.array, PropTypes.oneOf([null])]),
  contentComponent: PropTypes.oneOfType([
    PropTypes.func,
    PropTypes.oneOf([null]),
  ]),
  arrayOfElements: PropTypes.oneOfType([
    PropTypes.array,
    PropTypes.oneOf([null]),
  ]),
};
